...
Кастомный компонент чекбокса, сохраняющий состояние напрямую в `System settings` (в виде 1 или 0). Поддерживает сброс до значения по умолчанию долгим нажатием.
...
|------------------------|--------------|------------|----------------------------------------------------------------------------------------------------------------|
| `android:key`          | Да           | `String`   | Ключ системной настройки. По этому ключу в систему будет сохраняться 1 (true) или 0 (false) в формате Integer. |
