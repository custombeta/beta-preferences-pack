# betaListPreference
Компонент списка с диалогом выбора, сохраняющий выбранное значение напрямую в `System table` (в виде строки). Поддерживает сброс до значения по умолчанию долгим нажатием.
...
| `android:summary`      | Нет          | `String`       | Описание настройки под названием. Поддерживает форматирование: текущий пункт подставляется через `%s`.                                      |
...
| `android:dialogMessage`      | Нет          | `String`    | Текст сообщения внутри диалога, отображается над списком.                          |
| `android:dialogIcon`         | Нет          | `Drawable`  | Иконка в заголовке диалога.                                                        |
| `android:positiveButtonText` | Нет          | `String`    | Текст кнопки подтверждения (обычно «OK»).                                          |
| `android:negativeButtonText` | Нет          | `String`    | Текст кнопки отмены (обычно «Отмена»).                                             |
| `android:dialogLayout`       | Нет          | `Reference` | Ссылка на кастомный layout-ресурс для содержимого диалога.                         |
