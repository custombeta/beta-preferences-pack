# betaListPreference
Компонент списка с выделением выбранного пункта с помощью `RadioButton` в диалоге, сохраняющий выбранное значение напрямую в `System table` (в виде значения). Поддерживает сброс до значения по умолчанию долгим нажатием.
