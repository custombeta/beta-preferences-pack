...
Компонент текстового поля, сохраняющий введённое значение напрямую в `System table` (в виде строки). Поддерживает сброс до значения по умолчанию долгим нажатием.
...
| `android:summary`          | Нет          | `String`   | Описание настройки под названием. Поддерживает форматирование: текущее значение подставляется через `%s`.         |
